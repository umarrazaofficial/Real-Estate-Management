...
function handleResponse(response) {
  if (response.status === 401 && !trigger && getCookie(process.env.REACT_APP_PSP_TOKEN_COOKIE)) {
    trigger = true;
    clearCookie(process.env.REACT_APP_PSP_TOKEN_COOKIE);
    clearCookie(process.env.REACT_APP_PSP_ALLOWED_PAGES_COOKIE);
...
function get(url, debounce = false) {
  const headers = {
    ...
    authorization: `Bearer ${getCookie(process.env.REACT_APP_PSP_TOKEN_COOKIE)}`,
  };
